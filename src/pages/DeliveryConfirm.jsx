...
import { ArrowLeft, Camera, Pen, CheckCircle, RotateCcw, Upload, Package } from 'lucide-react'
...
export default function DeliveryConfirm() {
  ...
  const [step, setStep]   = useState('photo')      // 'photo' | 'signature' | 'done'
  ...
  const handleSubmit = async () => {
    setSubmitting(true)
    // TODO: Upload photo + signature canvas to Supabase Storage
    // const photoUrl = await uploadDeliveryPhoto(orderId, photoFile)
    // const sigBlob  = await new Promise(r => canvasRef.current.toBlob(r))
    // const sigUrl   = await uploadSignature(orderId, sigBlob)
  ...
  return (
    <div style={s.page}>
      ...
      <div style={s.content}>
        ...
        {step === 'photo' && (
          <div className="fade-up">
            <SectionHead icon={Camera} title="Foto da entrega"
              desc="Fotografe a entrega para registrar a comprovação." />
            ...
            {photo && (
              <button style={s.retakeBtn} onClick={() => setPhoto(null)}>
                <RotateCcw size={13} /> Tirar nova foto
              </button>
            )}

            <button
              style={{ ...s.primaryBtn, opacity: photo ? 1 : 0.38, marginTop: 24 }}
              disabled={!photo}
        ...
        {step === 'signature' && (
          <div className="fade-up">
            <SectionHead icon={Pen} title="Assinatura do cliente"
              desc="Peça ao cliente para assinar abaixo confirmando o recebimento." />
            ...
            <div style={s.canvasWrap}>
              ...
              {!signed && (
                <div style={s.canvasHint}>
                  <Pen size={22} color="var(--text-3)" />
                  <span style={s.hintTxt}>Assine aqui</span>
                </div>
              )}
            </div>

            <button style={s.retakeBtn} onClick={clearSig}>
              <RotateCcw size={13} /> Limpar assinatura
            </button>

            <button
              style={{ ...s.primaryBtn, opacity: signed && !submitting ? 1 : 0.38, marginTop: 24 }}
        ...
        {step === 'done' && (
          <div style={s.done} className="scale-in">
            ...
            <div style={s.doneSub}>
              ...
              <span style={{ color: 'var(--delivered)', fontWeight: 600 }}>Entregue</span> e o
              kanban foi atualizado automaticamente.
            </div>
...
const s = {
  ...
  backBtn: {
    ...
    color: 'var(--text-1)', display: 'flex', alignItems: 'center', flexShrink: 0,
  },
  ...
  retakeBtn: {
    ...
    fontSize: 13, padding: '8px 0', marginTop: 10, border: 'none',
  },
  ...
  canvasHint: {
    ...
    gap: 10, pointerEvents: 'none', opacity: 0.5,
  },
  ...
  primaryBtn: {
    ...
    borderRadius: 'var(--radius-sm)', fontSize: 15, fontWeight: 800,
  ...
  // Done screen
    ...
    textAlign: 'center', paddingTop: 40, gap: 18,
  },
  doneRing: {
    width: 96, height: 96, borderRadius: '50%',
    background: 'var(--delivered-bg)', border: '1px solid rgba(52,211,153,0.25)',
    display: 'flex', alignItems: 'center', justifyContent: 'center',
  },
  ...
  doneCard: {
    ...
    borderRadius: 'var(--radius-sm)', padding: '14px 16px', width: '100%', maxWidth: 320,
  },
}
