...
import { LogOut, Plus, Phone, MapPin, User, Package, Clock, CheckCircle, Truck, Search, X, ChevronRight } from 'lucide-react'
...
export default function AdminDashboard() {
  ...
  const handleCreate = async () => {
    setSaving(true)
    const { data, error } = await createPedido(newOrder)
    if (!error && data) {
            ...
            <div key={col} style={s.column}>
              ...
              <div style={s.cardList}>
                {cards.length === 0
                  ...
                  : cards.map((order, i) => (
                    <OrderCard key={order.id} order={order} delay={i*0.05} onMove={handleMove} />
                  ))
                }
              </div>
            </div>
        ...
        <div style={s.overlay} onClick={() => setShowModal(false)}>
          <div style={s.modal} onClick={e => e.stopPropagation()} className="scale-in">
            ...
            <div style={s.modalBody}>
              ...
              <button
                style={{ ...s.addBtn, width:'100%', justifyContent:'center', marginTop:4, opacity: saving ? 0.6 : 1 }}
  ...
  return (
    <div style={{ ...s.card, animationDelay:`${delay}s` }} className="slide-in">
      ...
      <div style={s.cardFoot}>
        ...
        {cfg.next && (
          <button style={s.advBtn} onClick={() => onMove(order.id, cfg.next)}>
...
function Row({ icon:Icon, text, truncate, accent }) {
  return (
    <div style={s.infoRow}>
      ...
      <span style={{ ...s.infoText, color: accent ? 'var(--accent)' : 'var(--text-2)', overflow: truncate ? 'hidden' : undefined, textOverflow: truncate ? 'ellipsis' : undefined, whiteSpace: truncate ? 'nowrap' : undefined, flex: truncate ? 1 : undefined }}>{text}</span>
    </div>
  )
}
...
const s = {
  ...
  card:{ background:'var(--bg-3)', border:'1px solid var(--border)', borderRadius:'var(--radius-sm)', padding:'11px 13px', display:'flex', flexDirection:'column', gap:6, opacity:0 },
