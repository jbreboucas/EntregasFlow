...
import { getPedidos, updatePedido, subscribePedidos } from '../lib/supabase'
import { LogOut, Package, MapPin, Phone, Truck, CheckCircle, Clock, Navigation, Link2, ChevronRight } from 'lucide-react'
...
export default function CourierDashboard() {
  ...
  useEffect(() => {
    ...
    const channel = subscribePedidos((payload) => {
      ...
      <div style={s.list}>
        {shown.length === 0
          ? <div style={s.empty} className="fade-in">
              ...
              <p style={s.emptySub}>{tab === 'available' ? 'Novos pedidos aparecerão aqui assim que forem criados.' : 'Associe um pedido da aba "Disponíveis".'}</p>
            </div>
          : shown.map((order, i) => (
            <CourierCard key={order.id} order={order} delay={i*0.05}
  ...
  return (
    <div style={{ ...s.card, animationDelay:`${delay}s` }} className="slide-in">
      ...
      <div style={s.cardFoot}>
        ...
        <div style={s.actions}>
          {isAvailable && <button style={s.associateBtn} onClick={onAssociate}><Link2 size={14} /> Associar</button>}
          {!isAvailable && order.status === 'em_rota' && <>
            <button style={s.mapBtn} onClick={onNavigate}><Navigation size={14} /> Rota</button>
            <button style={s.confirmBtn} onClick={onConfirm}><CheckCircle size={14} /> Confirmar <ChevronRight size={12} /></button>
          </>}
...
const s = {
  ...
  statsStrip:{ display:'flex', alignItems:'center', justifyContent:'center', gap:24, padding:'12px 18px', flexShrink:0, background:'var(--bg-2)', borderBottom:'1px solid var(--border)' },
  ...
  tabs:{ display:'flex', gap:2, padding:'8px 18px', background:'var(--bg-2)', borderBottom:'1px solid var(--border)', flexShrink:0 },
  ...
  empty:{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'60px 20px', gap:12, textAlign:'center' },
